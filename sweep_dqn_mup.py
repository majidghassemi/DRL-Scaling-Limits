...
# -------------------------
# 2) MuP-Style Model (The Fix)
# -------------------------
class MuP_DQN_Model(nn.Module):
    def __init__(self, d_in, n_hidden, d_out, act="relu"):
        ...
        # --- The MuP / Zero-Init Trick ---
        # W0: Standard He Init (Keeps activations O(1))
        nn.init.kaiming_normal_(self.W0.weight, nonlinearity=act)
        
        # W1: Initialize to ZERO
        # This removes the "noise" at the start. Q-values start at exactly 0.
        nn.init.zeros_(self.W1.weight)
        
        # A fixed scalar to boost the signal range to match CartPole rewards (~100)
        # This acts like a "readout gain" without changing the N-scaling dynamics
        self.output_mult = 10.0 

    def forward(self, x):
        h = self.act(self.W0(x))
        
        # MFT Scaling: (1/N)
        # We keep the 1/N to preserve the Feature Learning scaling law.
        # But we multiply by 'output_mult' to make the values usable for RL.
        out = (self.W1(h)) / self.n_hidden
...
def train_dqn(width, lr, steps=5000, bs=64, seed=0, gamma=0.99):
    ...
    policy_net = MuP_DQN_Model(4, width, 2).to(device)
    target_net = MuP_DQN_Model(4, width, 2).to(device)
    ...
    while total_steps < steps:
        epsilon = max(0.05, 1.0 - total_steps / 2000) # Slower decay
...
def plot_results(results, widths, alpha, outdir):
    ...
    plt.title(f"DQN MuP (Zero Init): Return vs Eta0 (alpha={alpha})")
    plt.legend(); plt.grid(True, which="both", ls=":")
    fig.savefig(outdir / f"dqn_mup_return_vs_eta0_alpha{alpha}.png", bbox_inches="tight")
    plt.close(fig)
...
def main():
    ...
    ap.add_argument("--steps", type=int, default=10000) # Increased steps
    ap.add_argument("--batch", type=int, default=64)
    ap.add_argument("--seeds", type=int, default=3)
    ap.add_argument("--outdir", type=str, default="out_dqn_mup")
    args = ap.parse_args()

    set_seed(0)
    base_etas = np.logspace(math.log10(args.eta0_min), math.log10(args.eta0_max), args.eta0_points)
    
    print(f"Sweeping DQN MuP | widths={args.widths} | alpha={args.alpha}")
    results = sweep(args.widths, base_etas, args.alpha, args.steps, args.batch, args.seeds)
    
    Path(args.outdir).mkdir(parents=True, exist_ok=True)
    with open(Path(args.outdir) / f"results_dqn_mup_alpha{args.alpha}.json", "w") as f:
