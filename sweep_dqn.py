...
def train_dqn(width, lr, steps=5000, bs=64, seed=0, gamma=0.99):
    ...
    policy_net = MFTwoLayer(4, width, 2).to(device)
    target_net = MFTwoLayer(4, width, 2).to(device)
    ...
    # Return negative return (so lower is better in plots)
    ...
    for n in widths:
        for eta0 in base_etas:
            ...
            for s in range(seeds):
                ...
                returns.append(-neg_r) # Flip back to positive for storage
            ...
            results.append({
                ...
                "eta0": float(eta0),
                "lr_raw": float(lr_raw),
                "val_loss": -float(mean_ret), # Store as negative for consistency with plot code
...
def plot_results(results, widths, alpha, outdir):
    ...
    # Plot Return vs Base Eta (Alignment)
    fig = plt.figure()
    for n in widths:
    ...
    plt.title(f"DQN MFT Scaling: Return vs Eta0 (alpha={alpha})")
    plt.legend(); plt.grid(True, which="both", ls=":")
    fig.savefig(outdir / f"dqn_return_vs_eta0_alpha{alpha}.png", bbox_inches="tight")
    plt.close(fig)
...
def main():
    ...
    ap.add_argument("--outdir", type=str, default="out_dqn_mft")
    args = ap.parse_args()

    set_seed(0)
    base_etas = np.logspace(math.log10(args.eta0_min), math.log10(args.eta0_max), args.eta0_points)
    print(f"Sweeping DQN MFT | widths={args.widths} | alpha={args.alpha}")
    results = sweep(args.widths, base_etas, args.alpha, args.steps, args.batch, args.seeds)
    
    Path(args.outdir).mkdir(parents=True, exist_ok=True)
    with open(Path(args.outdir) / f"results_dqn_alpha{args.alpha}.json", "w") as f:
